...
namespace WebApplications.Testing.Data
{
    ...
    public sealed class SqlMetaType
    {
        ...
        {
            if (!Is80Supported(type) && SqlDbType.Xml != type)
        ...
        {
            if (!Is90Supported(type) && SqlDbType.Date != type &&
        ...
        public static SqlMetaType GetMetaTypeFromSqlDbType(SqlDbType target, bool isMultiValued = false)
        {
            switch (target)
            {
                ...
                default:
                    throw new InvalidOperationException(string.Format("The '{0}' SqlDbType is unknown.", target));
            }
        }
