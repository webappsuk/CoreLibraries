    ...
    public class DecimalRangeTests : UtilitiesTestBase
    {
        ...
        public void DecimalRange_Iterating_DifferenceBetweenIterationsMatchesStepSize()
        {
            ...
            foreach (decimal i in decimalRange)
            {
                ...
                {
                    Assert.AreEqual((double)(i - previous.Value), (double)step, (double)step * 1e-6, "Difference between iteration values should match the step value supplied to within one millionth");
                }
                previous = i;
            }
        }
